...
use crate::consensus::{ConsensusRound, types::{CooperativeMetadata, ResourceImpact}};
use crate::websocket::WebSocketHandler;
use crate::vm::{VM, Contract, ExecutionContext, OpCode};
...
impl ICNCore {
    ...
    pub async fn start_consensus_round(&self) -> Result<(), String> {
        ...
        blockchain.consensus.start_round()?;
        
        if let Some(round) = blockchain.consensus.get_current_round() {
            // Broadcast event
